    ...
    public class DispatchReqSubConRepository : IDispatchReqSubConRepository {
        ...
        public ResponseMessage FBAssignVendor(FBDispatchReqSubCont fBDispatchReqSubCont)
        {
          ...
          public List<ComponentDetailsDto> GetStructrueComponent (int id ) {
                ...
                string strQuery = string.Format ("select dsc.id DispstructCompId, dsc.disp_structure_id DispStructureId,dmsc.qr_code ModStageQrCode ,c.qr_code QrCode,dmsc.id ModStageCompId,dmsc.leng ModStageLength,dmsc.weight ModStageWeight,dmsc.height ModStageHeight,dmsc.thickness ModStageThikness,dmsc.breath ModStagebreath,c.comp_name ComponentName,c.comp_id CompId,ct.name ComponentType,c.component_no ComponentNo,c.is_group IsGroup,c.drawing_no DrawingNo,c.leng Leng,c.breath Breath,c.height Height,c.thickness Thickness,c.weight Weight,c.make_type MakeType,c.is_tag IsTag from disp_structure_comp dsc inner join  component c  on  dsc.disp_comp_id  = c.id left outer join disp_mod_stage_component dmsc on dmsc.dispstruct_comp_id =dsc.id inner join component_type ct on c.comp_type_id =ct.id where dsc.disp_structure_id ={0}",id);
