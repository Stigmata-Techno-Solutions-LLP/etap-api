    ...
    public class AssignStructureComponentRepository : IAssignStructureComponentRepository {
        ...
        public List<AssignStructureDtlsOnly> GetAssignStructureDtls () {
            try {
               ...
               foreach (AssignStructureDtlsOnly item in result) {
                    List<ClassDrno> result1 = new List<ClassDrno> ();
                              string DrawingNo = string.Format("declare @results varchar(500) select @results = coalesce(@results + ',', '') +  convert(varchar(50),file_name) from project_structure_documents ps where ps.project_structure_id={0} order by file_name select @results as DrawingNo",item.ProjectStructureId);
                         result1 = _context.Set<ClassDrno>().FromSqlRaw(DrawingNo).ToList();
                         item.DrawingNo=result1.FirstOrDefault()?.DrawingNo;
                            }
