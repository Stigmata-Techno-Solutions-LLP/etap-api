...
AS BEGIN
	...
	SELECT @structureName = COALESCE(@structureName + ',','') + Name
	FROM structures
	where id in (select drs.proj_struct_id
...
BEGIN
	...
	select '1' as 'isAction', pp.name ProjectName, pp.proj_code ProjectCode, sitereq_id as SiteReqId, sd.Id as Id, sd.proj_struct_id as ProjStructId, s2.name as StructureName, ps.struct_code StructureCode, st.name as StructureTypeName, surplus_fromdate as SurplusDate, status as Status, status_internal as StatusInternal
	into #resultset1
	from site_declaration as sd inner join project_structure ps on sd.proj_struct_id =ps.structure_id inner join structures s2 on  ps.structure_id  = s2.id inner join project pp on pp.id =sd.from_project_id inner join structure_type st on s2.structure_type_id  = st.id
