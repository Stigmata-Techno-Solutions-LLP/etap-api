...
CREATE OR ALTER PROCEDURE sp_getDeclaration(
...
CREATE OR ALTER PROCEDURE sp_GetRequirement(
	@role_name varchar(50),
	@role_hierarchy int  null)
AS
BEGIN
...
BEGIN
	...
	BEGIN
		...
		BEGIN
				update project_structure set current_status ='READY TO REUSE' where id = (select proj_struct_id
...
CREATE   OR ALTER PROCEDURE sp_ApprovalRequirement(@req_id int,
	@role_name varchar(50),
	@role_hierarchy int  null
)
AS
...
CREATE      OR ALTER PROCEDURE sp_RejectRequirement(@req_id int,
	@role_name varchar(50),
	@role_hierarchy int  null
)
AS
...
CREATE    OR ALTER PROCEDURE [dbo].[SP_GETTWCCDispatch](@Status varchar(100))
AS
BEGIN
...
CREATE   OR ALTER PROCEDURE [dbo].[SP_TWCCInnerStructureDetails](@StructureId int)
AS
BEGIN
...
CREATE OR ALTER PROCEDURE SP_GetSiteRequirement(@SiteRequirementId int)
 AS
 BEGIN
...
END
	...
	CREATE   OR ALTER PROCEDURE SP_GetSubContractorComponentDetails(@DispatchStructureId int)
AS
BEGIN
...
CREATE OR ALTER PROCEDURE SP_GetViewStructureChartDetails(@ProjectStructureId int)
AS
BEGIN
